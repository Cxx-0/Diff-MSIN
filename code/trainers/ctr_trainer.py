...
class CTRTrainer(object):
    ...
    def __init__(
        ...
        self.CosSim_loss = torch.nn.CosineSimilarity()
    ...
    def train_one_epoch(self, data_loader, log_interval=10):
        ...
        for i, (x_dict, y) in enumerate(tk0):
            ...
            if (i + 1) % log_interval == 0:
                tk0.set_postfix(loss=(total_loss/ log_interval,total_gap_loss/ log_interval) )
                total_loss = 0
                total_gap_loss = 0
    ...
    def evaluate(self, model, data_loader):
        ...
        with torch.no_grad():
            ...
            for i, (x_dict, y) in enumerate(tk0):
                x_dict = {k: v.to(self.device) for k, v in x_dict.items()}
                y = y.to(self.device)
                y_pred,y_text_pre,y_image_pre,_,_ = model(x_dict)
